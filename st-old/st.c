    ...
    for (i = 0; i < 2; i++) {
        ...
        for (j = 0; j < term.row; ++j) {
            if (term.col != term.linelen)
                term.screen[i].buffer[j] = xrealloc(term.screen[i].buffer[j],
    ...
    if (linelen > term.linelen) {
        for (i = 0; i < term.screen[0].size; ++i) {
            if (term.screen[0].buffer[i]) {
                ...
                clearline(term.screen[0].buffer[i], term.c.attr, term.linelen,
                          linelen);
            }
        }
        for (i = 0; i < minrow; ++i) {
            ...
            clearline(term.screen[1].buffer[i], term.c.attr, term.linelen,
                      linelen);
        }
    }
